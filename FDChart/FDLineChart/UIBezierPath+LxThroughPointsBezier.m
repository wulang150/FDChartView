...
- (void)addBezierCurveToPoint:(CGPoint)tagetPoint
{
    if (self.isEmpty) {
        [self moveToPoint:tagetPoint];
        return;
    }
    
    CGPoint sPoint = self.currentPoint;
    CGPoint ePoint = tagetPoint;
    CGPoint mPoint = CenterPointOf(sPoint, ePoint);
    if (sPoint.x == ePoint.x && sPoint.y == ePoint.y) {
        return;
    }
    
    CGPoint controlPoint1 = CGPointMake(mPoint.x, sPoint.y);
    CGPoint controlPoint2 = CGPointMake(mPoint.x, ePoint.y);
    
    [self addCurveToPoint:ePoint controlPoint1:controlPoint1 controlPoint2:controlPoint2];
}
...
{
    return sqrt((point1.x - point2.x) * (point1.x - point2.x) + (point1.y - point2.y) * (point1.y - point2.y));//开方
}
